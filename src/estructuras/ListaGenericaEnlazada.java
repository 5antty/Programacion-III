...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	public ListaGenericaEnlazada<T> invertir() {
		this.comenzar();
		return this.reves(this);
		// Integer n = l.elemento(pos);

	}

	public ListaGenericaEnlazada<T> reves(ListaGenericaEnlazada<T> l) {
		ListaGenericaEnlazada<T> auxl = new ListaGenericaEnlazada<>();
		if (!l.fin()) {
			T aux = l.proximo();
			auxl.agregarInicio(aux);
			auxl = reves(l);
		}
		return auxl;
	}
